  ...
  module Importers
    class MastodonRSS < Importer
      ...
      def self.write_rss_item(item, options)
        ...
        filter_helper = Object.new.extend(Liquid::StandardFilters)

        time = item.date.strftime('%H:%M')
	pseudo_title = filter_helper.strip_html(item.description)
	  .split(' ')
	  .first(5)
	  .join(' ')
	  .chomp(',')
        ...
        post_name = Jekyll::Utils.slugify(time + pseudo_title, :mode => "latin")
	...
	if item.link 
	  output << <<~SOURCE.strip
            <p>via <a href="#{item.link}">brainwashed@nrw.social</a></p>
          SOURCE
	end
